...
--
-- Servidor: 127.0.0.1
-- Tiempo de generación: 22-07-2022 a las 22:30:32
...
INSERT INTO `pago` (`id_pago`, `contrato_id`, `fecha_registro`) VALUES
(1, 1, '');
...
CREATE TABLE `persona` (
  ...
  `rol_arrendatario` int(3) NOT NULL DEFAULT 0,
  `rol_arrendador` int(3) NOT NULL DEFAULT 0,
  `fecha_registro` timestamp NULL DEFAULT current_timestamp()
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4;
...
INSERT INTO `persona` (`id_persona`, `nombre`, `apellido`, `cedula`, `celular`, `direccion`, `rol_arrendatario`, `rol_arrendador`, `fecha_registro`) VALUES
...
(3, 'maria', 'cifuentes', '446262462', '3112245368', 'mz 14 casa22 Grisales', 1, 0, '0000-00-00 00:00:00'),
(666, 'carlos', 'martinez', '1090343457', '3224534566', 'mz 14 casa22 Campo Alegre', 1, 1, NULL),
(777, 'maria', 'angustina', '1090342367', '3224567677', 'mz 14 casa22 Conjunto las flores', 1, 0, '2022-07-22 20:03:27'),
(888, 'ridrigo', 'bonillla', '109023456', '3224567899', 'mz 14 casa 34 Monte Alegre', 1, 0, NULL),
(1111, 'camilo', 'daza', '1090344656', '3224567856', 'mz 14 casa22 centro', 0, 0, '0000-00-00 00:00:00'),
(4444, '444', '444', '4444', '444', '4444', 0, 0, NULL),
(4445, '999', '999', '999', '999', '9999', 1, 0, '2022-07-22 19:59:30'),
...
--
DELIMITER $$
CREATE TRIGGER `insertar_arrendador` AFTER INSERT ON `persona` FOR EACH ROW IF NEW.rol_arrendador = 1 THEN
INSERT INTO `arrendador`(`id_arrendador`, `persona_id`) VALUES (Null, NEW.id_persona);
END IF
$$
DELIMITER ;
DELIMITER $$
CREATE TRIGGER `insertar_arrendatario` AFTER INSERT ON `persona` FOR EACH ROW IF NEW.rol_arrendatario = 1 THEN
...
CREATE ALGORITHM=UNDEFINED DEFINER=`root`@`localhost` SQL SECURITY DEFINER VIEW `vista_arrendadores`  AS SELECT `arrendador`.`id_arrendador` AS `id_arrendador`, `persona`.`nombre` AS `nombre`, `persona`.`apellido` AS `apellido`, `persona`.`cedula` AS `cedula`, `persona`.`celular` AS `celular`, `persona`.`direccion` AS `direccion`, `persona`.`fecha_registro` AS `reg_peronsa`, `arrendador`.`fecha_registro` AS `reg_arrendador` FROM (`arrendador` join `persona`) WHERE `arrendador`.`persona_id` = `persona`.`id_persona``id_persona`  ;
...
--
ALTER TABLE `persona`
  MODIFY `id_persona` int(11) NOT NULL AUTO_INCREMENT, AUTO_INCREMENT=5557;
